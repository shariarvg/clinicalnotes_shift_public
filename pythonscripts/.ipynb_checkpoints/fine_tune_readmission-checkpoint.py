...
ep = MIMICEndpoint()
n = ep.get_notes_start_year(2016)
notes = pd.concat([ep.get_notes_readmission(1, 1500, notes = n), ep.get_notes_readmission(0, 1500, notes = n)])

V = "4"

notes[['note_id']].to_csv(f"gatortron_readmission_classifer_note_ids_V{V}.csv")
...
class NotesDataset(torch.utils.data.Dataset):
    def __init__(self, data):
        ...
        self.labels = list(data['admission_in_30_days'])
...
for epoch in range(n_epochs):
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), f"gatortron_readmission_classifier_chkpt_epoch{epoch}_V{V}.pt")
    
torch.save(model.state_dict(), f"gatortron_readmission_classifier_V{V}.pt")
