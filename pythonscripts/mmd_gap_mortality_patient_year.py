'''
Obtain a bunch of dataset pairs from the same source pair, then
1) Featurize both datasets (preproc for prediction)
2) Embed both datasets (preproc for MMD)
3) Train a model on one dataset featurization, eval on the other dataset's featurization, AUC
4) Find MMD between them using embedding
'''
