...
text = '''
Sources are start years.

For each row in output, there are columns eval_assn (which represents the year of the eval dataset), mmd2008...mmd2020 (which represent the mmd's of each ref against the eval), mauve2008...mauve2020 (which represent the mauve's of each ref against the eval), and then binary MMDCorrect, MauveCorrect

Coming from source_recovery_script.py
'''
...
save_name = "../experimentresults/yeargroup_source_recovery"

data = []
columns = ["EvalAssn"] + [f"MMD{year}" for year in years] + [f"Mauve{year}" for year in years] + ["MMDCorrect", "MauveCorrect"]
...
for count in range(N_trials):
    ...
    sources = [MIMICSource(ep, "get_notes_start_year", year) for year in years]
    ...
    print(metrics)
    data.append([assignment] + metrics[0] + metrics[1] + [mmd_correct, mauve_correct])
