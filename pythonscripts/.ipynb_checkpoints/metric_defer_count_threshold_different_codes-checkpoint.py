...
def get_correct_assignment(model, summary, max_length, save_name, NUM_TRAIN = 15, TRAIN_SIZE = 100, EVAL_SIZE = 75, N_EVAL_PER_TRAIN = 20, best_score = "Min"):
    ...
    for code in codes:
        for count in range(N_EVAL_PER_TRAIN):
            ...
            all_divs.append([ep.count_threshold(set(eval_df['hadm_id']), hadm_ids, 2, 10) for hadm_ids in train_df_hadm_id_sets])
    data = np.hstack((np.array(assignments).reshape(-1,1), np.array(all_metrics), np.array(all_divs)))

    columns = ["Assignment"] + [f"Metric_{code}" for code in codes] + [f"Div_{code}" for code in codes]
...
save_name = "mmd_count_threshold" + "_" + model + "_" + summary + "_" + str(max_length) + "_" + "multi_code"
